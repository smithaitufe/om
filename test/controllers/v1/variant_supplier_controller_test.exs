defmodule Store.V1.VariantSupplierControllerTest do
  ...
  test "lists all entries on index", %{conn: conn} do
    conn = get conn, v1_variant_supplier_path(conn, :index)
    assert json_response(conn, 200)["data"] == []
  end

  test "shows chosen resource", %{conn: conn} do
    variant_supplier = Repo.insert! %VariantSupplier{}
    conn = get conn, v1_variant_supplier_path(conn, :show, variant_supplier)
  ...
  test "does not show resource and instead throw error when id is nonexistent", %{conn: conn} do
    assert_raise Ecto.NoResultsError, fn ->
      get conn, v1_variant_supplier_path(conn, :show, -1)
    end
  end

  test "creates and renders resource when data is valid", %{conn: conn} do
    conn = post conn, v1_variant_supplier_path(conn, :create), variant_supplier: @valid_attrs
    assert json_response(conn, 201)["data"]["id"]
    assert Repo.get_by(VariantSupplier, @valid_attrs)
  end

  test "does not create resource and renders errors when data is invalid", %{conn: conn} do
    conn = post conn, v1_variant_supplier_path(conn, :create), variant_supplier: @invalid_attrs
    assert json_response(conn, 422)["errors"] != %{}
  end

  test "updates and renders chosen resource when data is valid", %{conn: conn} do
    variant_supplier = Repo.insert! %VariantSupplier{}
    conn = put conn, v1_variant_supplier_path(conn, :update, variant_supplier), variant_supplier: @valid_attrs
  ...
  test "does not update chosen resource and renders errors when data is invalid", %{conn: conn} do
    variant_supplier = Repo.insert! %VariantSupplier{}
    conn = put conn, v1_variant_supplier_path(conn, :update, variant_supplier), variant_supplier: @invalid_attrs
    assert json_response(conn, 422)["errors"] != %{}
  end

  test "deletes chosen resource", %{conn: conn} do
    variant_supplier = Repo.insert! %VariantSupplier{}
    conn = delete conn, v1_variant_supplier_path(conn, :delete, variant_supplier)
