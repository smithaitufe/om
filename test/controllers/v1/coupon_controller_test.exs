defmodule Store.V1.CouponControllerTest do
  ...
  test "does not show resource and instead throw error when id is nonexistent", %{conn: conn} do
    assert_raise Ecto.NoResultsError, fn ->
      get conn, coupon_path(conn, :show, -1)
    end
  end

  test "creates and renders resource when data is valid", %{conn: conn} do
    conn = post conn, coupon_path(conn, :create), coupon: @valid_attrs
  ...
  test "updates and renders chosen resource when data is valid", %{conn: conn} do
    coupon = Repo.insert! %Coupon{}
    conn = put conn, coupon_path(conn, :update, coupon), coupon: @valid_attrs
